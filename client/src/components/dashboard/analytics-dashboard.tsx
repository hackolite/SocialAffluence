...
export default function AnalyticsDashboard({
  ...
  const chartData = timelineData.slice(-60).map((item) => ({
    ...
    people: item.people,
    others: Math.max(item.total - item.people, 0), // pour éviter des valeurs négatives
  }));

  // Calculer le total de personnes
  const totalPeople = timelineData.reduce((sum, item) => sum + item.people, 0);

  // Calculer les classes cumulées
      ...
      if (item.classCounts) {
        Object.entries(item.classCounts).forEach(([className, count]) => {
          finalClassCounts[className] =
            (finalClassCounts[className] || 0) + count;
        });
      }
    });
  }

  // Top 5 des classes les plus détectées
              ...
              {topClasses.length > 0 ? (
                <div className="space-y-3">
                  <div className="text-sm text-slate-300 font-medium">
                    Top 5 Classes Détectées:
                  </div>
                  {topClasses.map(([className, count]) => {
                    const total = Object.values(finalClassCounts).reduce(
                      (sum, c) => sum + c,
                      0
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 bg-slate-700 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full transition-all duration-500 ${getClassColor(
                              className
                            )}`}
